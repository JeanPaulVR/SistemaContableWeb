...
@WebServlet(name = "RegistroInvBalCuenta42", urlPatterns = {"/RegistroInvBalCuenta42"})
public class RegistroInvBalCuenta42 extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String[] columnElements = {"INFORMACIÓN DEL CLIENTE", "", "", "MONTO DE LA CUENTA POR COBRAR",
            "FECHA DE EMISIÓN DEL COMPROBANTE DE PAGO"};

        String[] columnElements1 = {"DOCUMENTO DE IDENTIDAD", "", "APELLIDOS Y NOMBRES, DENOMINACIÓN O RAZÓN SOCIAL",
            "", ""};

        String[] columnElements2 = {"TIPO (TABLA 2)", "NÚMERO", "", "", ""};
        ...
        cell1.setCellValue("FOMATO 3.12: \"LIBRO DE INVENTARIOS Y BALANCES - DETALLE DEL SALDO DE LA CUENTA 42- CUENTAS POR PAGAR COMERCIALES TERCEROS\"");
        ...
        for (int i = 0; i < 10; i++) {
            String cuenta = 42 + "" + i;
            libros = libroDao.InventarioBalances(id, cuenta);

            if (!libros.isEmpty()) {
                String cuentaS = String.valueOf(libros.get(0).getCuentaCC());
                String substring = cuentaS.substring(0, cuentaS.length() - 1);
                ...
                for (int j = 0; j < num; j++) {
                    ...
                    ccBeans = ccLogic.DatosCC(Integer.parseInt(substring));

                    if (substring.length() > 2) {

                        row = sheet.createRow(8 + num - j);
                        cell1 = row.createCell(3);
                        cell1.setCellValue(ccBeans.getNumero() + "-" + ccBeans.getNombre());
                ...
                for (int k = 0; k < libros.size(); k++) {
                    ...
                    row = sheet.createRow(9 + num + k);
                    cell1 = row.createCell(1);
                    cell1.setCellValue(libros.get(k).getTipoidenti());
                    cell1.setCellStyle(style3);
                    cell1 = row.createCell(2);
                    cell1.setCellValue(libros.get(k).getNumeroidenti());
                    cell1.setCellStyle(style3);
                    cell1 = row.createCell(3);
                    cell1.setCellValue(libros.get(k).getDenominacion());
                    cell1.setCellStyle(style3);
                    cell1 = row.createCell(4);
                    cell1.setCellValue(libros.get(k).getHaber());
                    cell1.setCellStyle(style3);
                    sum++;
                }

                for (int l = 0; l < sum; l++) {
                    row = sheet.createRow(9 + num + libros.size());
                    cell1 = row.createCell(3);
                    cell1.setCellValue("TOTAL");
                    cell1.setCellStyle(style1);
                    cell1 = row.createCell(4);
                    cell1.setCellFormula("SUM(E" + (13  + sum) + ":E"+ (14 + pos) + ")");
                    cell1.setCellStyle(style1);
                }
        ...
        response.setHeader("Content-Disposition", "attachment; filename=RegistroInvBalCuenta42.xlsx");
