...
@WebServlet(name = "RegistroCompras", urlPatterns = {"/RegistroCompras"})
public class RegistroCompras extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        XSSFSheet sheet = workbook.createSheet("Compras");
        ...
        String[] columnElements = {"NUMERO CORRELATIVO DEL REGISTRO O CODIGO UNICO DE LA OPERACION",
            ...
            "COMPROBANTE DE PAGO O DOCUMENTO", "", "", "Nº DEL COMP. DE PAGO, DOC, N° DE ORDEN DEL FORMULARIO FISICO O VIRTUAL, Nº DE DUA, DSI O LIQUIDACION DE COBRANZA U OTROS DOCS. EMITIDOS",
            "INFORMACION DEL PROVEEDOR", "", "", "ADQUISICIONES GRAVADAS DESTINADAS A OPERACIONES GRAVADAS Y/O DE EXPORTACION",
            ...
            "DIFERENCIA DE CAMBIO", "ISC", "OTROS TRIBUTOS Y CARGOS", "IMPORTE TOTAL", "N° DE COMPROBANTE DE PAGO EMITIDO POR SUJETO NO DOMICILIADO",
            "CONSTANCIA DE DEPOSITO DE DETRACCION", "", "TIPO DE CAMBIO", "REFERENCIA DEL COMPROBANTE DE PAGO O DOCUMENTO ORIGINAL QUE SE MODIFICA",
            "", "", ""};

        String[] columnElements1 = {"", "", "", "TIPO(TABLA 10)", "SERIE O CODIGO DE LA DEPENDENCIA ADUANERA", "AñO DE EMISION DE LA DUAO DSI",
        ...
        String[] columnElements2 = {"", "", "", "", "", "", "", "TIPO(TABLA 2)", "NUMERO", "", "BASE IMPONIBLE", "IGV",
            "BASE IMPONIBLE", "IGV", "BASE IMPONIBLE", "IGV", "", "", "", "", "", "", "NUMERO", "FECHA DE EMISION", "", "",
            "", "", ""};
        ...
        cell1.setCellValue("FORMATO 8.1: REGISTRO DE COMPRAS");
        ...
        sheet.setColumnWidth(10, 256 * 45);
        ...
        sheet.setColumnWidth(20, 256 * 12);
        ...
        sheet.setColumnWidth(26, 256 * 20);
        sheet.setColumnWidth(27, 256 * 20);
        sheet.setColumnWidth(28, 256 * 20);
        sheet.setColumnWidth(29, 256 * 20);
        ...
        libros = libroDao.listarCompras(fi, ff, id);
        XSSFCell celda;
        for (int i = 0; i < libros.size(); i++) {
            ...
            celda.setCellValue(libros.get(i).getBIAGCFOGE());
            celda.setCellStyle(style3);
            celda = row.createCell(12);
            celda.setCellValue(libros.get(i).getMIGVIPMOPG());
            celda.setCellStyle(style3);
            celda = row.createCell(13);
            celda.setCellValue(libros.get(i).getBIAGCFOGyNG());
            celda.setCellStyle(style3);
            celda = row.createCell(14);
            celda.setCellValue(libros.get(i).getMIGVIPMOPGyNG());
            celda.setCellStyle(style3);
            celda = row.createCell(15);
            celda.setCellValue(libros.get(i).getBIAGsinCF());
            celda.setCellStyle(style3);
            celda = row.createCell(16);
            celda.setCellValue(libros.get(i).getMIGVIPMsinCF());
            celda.setCellStyle(style3);
            celda = row.createCell(17);
            celda.setCellValue(libros.get(i).getVANG());
            celda.setCellStyle(style3);
        ...
        if (!libros.isEmpty()) {
            ...
            celda.setCellStyle(style1);
            celda = row.createCell(18);
            celda.setCellFormula("SUM(S" + (11 + sum) + ":S" + (10 + pos) + ")");
            celda.setCellStyle(style1);
            celda = row.createCell(19);
            celda.setCellFormula("SUM(T" + (11 + sum) + ":T" + (10 + pos) + ")");
            celda.setCellStyle(style1);
            celda = row.createCell(20);
            celda.setCellFormula("SUM(U" + (11 + sum) + ":U" + (10 + pos) + ")");
            celda.setCellStyle(style1);
            celda = row.createCell(21);
            celda.setCellFormula("SUM(V" + (11 + sum) + ":V" + (10 + pos) + ")");
            celda.setCellStyle(style1);
        }

        // Establecer la respuesta HTTP para descargar el archivo
        response.setContentType("application/vnd.ms-excel");
        response.setHeader("Content-Disposition", "attachment; filename=RegistroCompras.xlsx");
